...
# # Testing model against the entire dataset (all site data combined) using TFCV
# ALL_data <- read.csv("ALL_data_reduced.csv")
# 
# 
# # Turn into factors
# temp <- select(ALL_data, -Product_Charge_Price)
# temp[,(1:ncol(temp))] <- lapply(temp[,(1:ncol(temp))],as.factor)
# nums <- select(ALL_data, Product_Charge_Price)
# ALL_data <- cbind(temp, nums)
# 
# 
# 
# pred <- predict(model_fit, newdata = ALL_data)
# conf <- confusionMatrix(data = pred, ALL_data$fraud_status)
# conf
# conf$byClass
