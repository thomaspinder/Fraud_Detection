...
glm_data <- select(glm_data, -Category_Level_2Accessories,
                   -Category_Level_2Bags, -Category_Level_2Clothing,
                   -Category_Level_2Footwear, -Category_Level_2Gifting)


glm_data <- select(glm_data, -Campaign_Key, -occupation_int, 
                   -Delivery_Option_Type_Key, -Order_Payment_Status_Key,
                   -Payment_Method_Key, -Medium_Key, -Payment_Provider_Key,
                   -pay_key_num, -destination_int, -canc_prop,
                   -count, -priority_int, -Cancelled_Qty,
                   -ship_status, -num_valid, -Ordered_Qty)
...
set.seed(123)
train <- SMOTE(fraud_status ~., new_data, perc.over = 20000, perc.under = 125)
